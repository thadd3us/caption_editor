...
test.describe('Drag-and-drop with webUtils.getPathForFile()', () => {
  test('should expose getPathForFile API (note: returns empty for synthetic File objects)', async ({ page }) => {
    ...
    const testCaptionsPath = path.join(getProjectRoot(), 'test_data/sample.captions_json5.copy')
    ...
    try {
      ...
      const result = await page.evaluate(async (filePath) => {
        ...
        const file = new File([blob], filePath.split('/').pop() || 'test.captions_json5', {
  ...
  test('should handle multiple files dropped simultaneously', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const files = [
        new File([JSON.stringify({ metadata: { id: 'doc1' }, segments: [{ id: 'cue1', startTime: 1, endTime: 2, text: 'Test 1' }] }, null, 2)], 'test1.captions_json5', { type: 'application/json' }),
        new File([JSON.stringify({ metadata: { id: 'doc2' }, segments: [{ id: 'cue1', startTime: 1, endTime: 2, text: 'Test 2' }] }, null, 2)], 'test2.captions_json5', { type: 'application/json' })
      ]
